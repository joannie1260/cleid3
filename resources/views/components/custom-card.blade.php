@props(['title', 'amount', 'percentage', 'comparison', 'data'])

<div class="flex flex-col h-min py-8 px-6 space-y-6 rounded-xl border border-gray-200 bg-white">
    {{ $slot }}
    ...
    <div>
        ...
        <div class="mt-2 flex justify-center gap-4">
            <h3 class="text-3xl font-bold text-gray-700">${{ $amount }}</h3>
            <div class="flex items-end gap-1 text-green-500">
                <svg class="w-3" viewBox="0 0 12 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M6 0L12 8H0L6 0Z" fill="black"/>
                </svg>
                <span>{{ $percentage }}%</span>
            </div>
        </div>
        <span class="block text-center text-gray-500">Compared to last week ${{ $comparison }}</span>
    </div>

    <table class="h-20 text-gray-600">
        <tbody>
            @foreach($data as $item)
                <tr class="h-20">
                    <td class="py-2">{{ $item['label'] }}</td>
                    <td class="text-gray-500">{{ $item['value'] }}</td>
                    <td>{!! $item['icon'] !!}</td>
                </tr>
            @endforeach
        </tbody>
